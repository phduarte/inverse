...
public class Column : Entity<string>, IColumn, IVisualElement
{
    ...
    public int Width => Table?.Width ?? 0;

    public int Height => HEIGHT;

    public int Left => Table?.Left ?? 0;

    public int Right => Table?.Right ?? 0;

    public int Top => (Table?.Top ?? 0) + Index * 30;

    public int Bottom => Top + Height;

    public int Center => Table?.Center ?? 0;
